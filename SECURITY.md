...
- Generic denial-of-service reports outside the built-in authentication limiter, or "I sent 100k
  requests and it got slow". Broader traffic shaping still belongs at the network edge. Genuine
  amplification (one small request causing unbounded work) *is* in scope.
...
- **Production fails closed.** With `NODE_ENV=production`, startup requires an HTTPS origin, a
  compatible RP ID, an absolute data directory, mandatory WebAuthn user verification and a
  loopback bind. The inherited container deployment uses an explicit non-loopback exception only
  on its isolated bridge network. Unsafe requests must carry the exact configured `Origin`, and
  the four passkey registration/login endpoints share a per-client fixed-window rate limit.
...
- **Development mode is intentionally less strict.** Outside `NODE_ENV=production`, an `Origin`
  header is not required and WebAuthn user verification defaults to `preferred` so localhost
  development remains practical. Never expose that mode to the internet.
...
- **HTTPS is required and the app doesn't provide it.** The API container speaks plain HTTP and
  ...
  in the clear.
- **Rate limiting is deliberately narrow.** Registration and login are limited per client IP,
  with `AUTH_RATE_LIMIT_MAX` requests per `AUTH_RATE_LIMIT_WINDOW_SECONDS`. Other authenticated
  writes have no application-level request quota. New invite codes contain 64 random bits; revoke
  and reissue unused 32-bit codes generated by older versions. Request bodies are capped at 5 MB.
- **A few endpoints answer without a session:** `/api/health`, `/api/config` (whether invite-only
  is on), `/api/push/public-key`, and the register/login handshakes. The health response does not
  disclose the user count.
