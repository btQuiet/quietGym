...
//
// Two jobs:
//  1. A small, self-contained file a friend can import into THEIR quietGym — just the
...
export function parsePlan(raw) {
  ...
  if (!data || !data.opengym_plan || !Array.isArray(data.routines)) {
    throw new Error(t('this isn’t a quietGym plan file'))
  }
...
<body><div class="doc">
  ...
  ${body}
  <footer>${esc(t('Made with quietGym'))}</footer>
</div></body></html>`
}
