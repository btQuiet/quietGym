...
export default function Settings() {
  ...
  const doExport = async () => {
    ...
    const name = 'quietgym-backup-' + todayISO() + '.json'
  ...
  const doImport = ev => {
    ...
    rd.onload = () => {
      try {
        ...
        if (!data.workouts || !data.routines) throw new Error('not a quietGym backup')
  ...
  // Ends the owner's sessions on every device — this one included, so on success it lands in
  ...
  const signOutEverywhere = () => confirmSheet({
    ...
    message: t('Signs the account out on every device, including this one. Sign in with your password again anytime.'),
  ...
  return <div className="narrow">
    ...
    <Section title={MOBILE ? t('Your data') : DEMO ? t('Demo') : t('Account')}>
      {MOBILE ? <>
        ...
        <Row icon="rocket" iconTint="var(--indigo)" title={t('Self-host quietGym')} subtitle={t('Password sign-in, sync across your devices, your own data.')} accessory="chevron"
        ...
        <Row icon="personCircle" iconTint="var(--grey)" title={t('Owner account')} subtitle={t('Signed in — data syncs across your devices.')} />
        <Row icon="signOut" iconTint="var(--red)" title={t('Sign out')} danger onClick={() => confirmSheet({ title: t('Sign out?'), message: t('Your data is synced to the server first, then cleared from this device.'), confirmText: t('Sign out'), danger: true, onConfirm: () => { signOut(); nav('/home') } })} />
        <Row icon="shield" iconTint="var(--red)" title={t('Sign out everywhere')} subtitle={t('Ends the account sessions on all your devices.')} danger onClick={signOutEverywhere} />
      </> : (
        <Row icon="lock" iconTint="var(--grey)" title={t('Sign in required')} />
      )}
    </Section>
    ...
    {/* ---------- appearance ---------- */}
    <Section title={t('Appearance')} footer={DEMO || MOBILE ? undefined : t('synced with your account')}>
      <Row icon="moon" iconTint="var(--indigo)" title={t('Theme')}>
        <Segmented
          ...
          options={[
            ...
            { value: 'qs', icon: 'sparkles', label: 'QS' }
          ]}
          value={['dark', 'light', 'qs'].includes(S.theme) ? S.theme : 'dark'}
      ...
      {S.theme !== 'qs' && <div className="lrow" style={{ flexDirection: 'column', alignItems: 'stretch', gap: 12, paddingTop: 13, paddingBottom: 14 }}>
    ...
    {!MOBILE && <Section title={t('Tip')}>
      <Row icon="lightbulb" iconTint="var(--yellow)"
        ...
        subtitle={t('to install quietGym as a full-screen app.') + ' ' + (user ? t('Your data syncs with your account — sign in anywhere to see it.') : t('Guest data stays on this device — export a backup now and then!'))} />
    </Section>}

    <div className="dim small" style={{ textAlign: 'center', marginTop: 4, lineHeight: 1.6 }}>
      quietGym · {t('free & open source (AGPL v3)')}<br />
      <a href="https://github.com/DuarteSantos8" target="_blank" rel="noopener">Duarte Santos</a> · <a href="https://github.com/btQuiet" target="_blank" rel="noopener">btQuiet</a><br />
      <a href="https://github.com/btQuiet/quietGym" target="_blank" rel="noopener">source code</a> · exercise data: hasaneyldrm/exercises-dataset (CC)
    </div>
  </div>
}
  ...
  return <>
    <Section
      ...
      footer={on && S.reminder?.on
        ...
        : null}
    >
      <Row icon="bell" iconTint="var(--red)" title={t('Push notifications')} subtitle={t('Rest-timer alerts, even if quietGym is closed.')}>
